...
// 1. GỌI HEADER (Header đã lo kết nối Config & DB)
require_once '../Layout/header.php';

// 2. XỬ LÝ UPDATE TRẠNG THÁI (Giữ nguyên logic của bạn)
$message = '';
...
// 3. XỬ LÝ LỌC & QUERY (Giữ nguyên logic của bạn)
...
if (!$result) {
    $message .= "<div class='alert alert-warning'>Lỗi SQL: " . $conn->error . "</div>";
}
?>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Chi tiết Sản phẩm #<span id="productIdDisplay"></span></h5>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-4">
                        <img src="<?php echo BASE_URL; ?>/images/placeholder.png" class="img-fluid rounded border" alt="Ảnh sản phẩm">
                    </div>
                    <div class="col-md-8">
...
<script>
    $('.quick-approve-btn').on('click', function(e) {
        ...
        var form = $('<form action="" method="post"><input type="hidden" name="pid" value="' + pid + '" /><input type="hidden" name="new_status" value="' + status + '" /><input type="hidden" name="update_product_status" value="1" /></form>');
        $('body').append(form);
        form.submit();
    });
